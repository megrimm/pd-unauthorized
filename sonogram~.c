...
static void *sonogram_do_update_part(void *tdata)
{
    ...
    SYS_VGUI3(".x%lx.c itemconfigure %lxSONOGRAM -outline #000000\n", canvas, x);
    if ( x->x_phaso )
    {
        SYS_VGUI3(".x%lx.c itemconfigure %lxPHASOGRAM -outline #000000\n", canvas, x);
    }
                                 ...
                                 t_int erase, t_int redraw, t_int keepframe)
{
    ...
    if ( x->x_graphic )
    {
        ...
        {
            SYS_VGUI3(".x%lx.c delete %lxSONOGRAM\n", canvas, x );
            SYS_VGUI7(".x%lx.c create rectangle %d %d %d %d -fill #FFFFFF -tags %xSONOGRAM\n",
                      ...
                      x);
            SYS_VGUI2("image delete SONIMAGE%lx\n", x );
            SYS_VGUI3(".x%lx.c delete ISONIMAGE%lx\n", canvas, x );
            SYS_VGUI4("image create photo SONIMAGE%lx -format gif -width %d -height %d\n",
                      x, x->x_size*x->x_zoom, x->x_blocksize/2*x->x_zoom );
            SYS_VGUI2("SONIMAGE%lx blank\n", x);
            SYS_VGUI6(".x%lx.c create image %d %d -image SONIMAGE%lx -tags ISONIMAGE%x\n",
                      canvas, x->x_xpos+((x->x_size*x->x_zoom)/2),
                      (x->x_ypos+((x->x_blocksize/2*x->x_zoom)/2)), x, x );
            if ( x->x_phaso )
            {
                SYS_VGUI3(".x%lx.c delete %lxPHASOGRAM\n", canvas, x );
                SYS_VGUI7(".x%lx.c create rectangle %d %d %d %d -fill #FFFFFF -tags %lxPHASOGRAM\n",
                          canvas, x->x_xpos-1, x->x_ypos+x->x_blocksize/2*x->x_zoom+2,
                          x->x_xpos + x->x_size*x->x_zoom +1,
                          x->x_ypos + x->x_blocksize*x->x_zoom + 3,
                          x);
                SYS_VGUI2("image delete FAZIMAGE%lx\n", x );
                SYS_VGUI3(".x%lx.c delete IFAZIMAGE%lx\n", canvas, x );
                SYS_VGUI4("image create photo FAZIMAGE%lx -format gif -width %d -height %d\n",
                          x, x->x_size*x->x_zoom, x->x_blocksize/2*x->x_zoom );
                SYS_VGUI2("FAZIMAGE%lx blank\n", x);
                SYS_VGUI6(".x%lx.c create image %d %d -image FAZIMAGE%lx -tags IFAZIMAGE%lx\n",
                          canvas, x->x_xpos+((x->x_size*x->x_zoom)/2),
        ...
        // set borders in red
        SYS_VGUI3(".x%lx.c itemconfigure %lxSONOGRAM -outline #FF0000\n", canvas, x);
        if ( x->x_phaso )
        {
            SYS_VGUI3(".x%lx.c itemconfigure %lxPHASOGRAM -outline #FF0000\n", canvas, x);
        }
...
static void sonogram_paste_phase( t_sonogram* x)
{
    ...
    // start a new capture
    SYS_VGUI3( ".x%lx.c delete %lxCAPTURE\n", canvas, x );
...
static void sonogram_draw_new(t_sonogram *x, t_glist *glist)
{
    ...
    if ( x->x_graphic )
    {
        SYS_VGUI7(".x%lx.c create rectangle %d %d %d %d -fill #FFFFFF -tags %lxSONOGRAM\n",
        ...
        SYS_VGUI6(".x%lx.c create image %d %d -image SONIMAGE%lx -tags ISONIMAGE%lx\n",
                  canvas, x->x_xpos+((x->x_size*x->x_zoom)/2),
                  (x->x_ypos+((x->x_blocksize/2*x->x_zoom)/2)), x, x );
        if ( x->x_phaso )
        {
...
static void sonogram_draw_delete(t_sonogram *x, t_glist *glist)
{
    ...
    {
        SYS_VGUI3( ".x%lx.c delete %lxCAPTURE\n", canvas, x );
        SYS_VGUI3( ".x%lx.c delete line %lxREADSTART\n", canvas, x);
        SYS_VGUI3( ".x%lx.c delete line %lxREADEND\n", canvas, x);
        SYS_VGUI3( ".x%lx.c delete line %lxMODSTART\n", canvas, x);
        SYS_VGUI3( ".x%lx.c delete line %lxMODEND\n", canvas, x);
        SYS_VGUI3(".x%lx.c delete %lxSONOGRAM\n", canvas, x );
        SYS_VGUI3(".x%lx.c delete %lxPHASOGRAM\n", canvas, x );
        SYS_VGUI3(".x%lx.c delete %lxISONIMAGE\n", canvas, x );
        SYS_VGUI2("image delete SONIMAGE%lx\n", x );
        if ( x->x_phaso )
        {
            SYS_VGUI3(".x%lx.c delete %lxIFAZIMAGE\n", canvas, x );
            SYS_VGUI2("image delete FAZIMAGE%lx\n", x );
        }
    }
}
...
static void sonogram_draw_move(t_sonogram *x, t_glist *glist)
{
    ...
    {
        SYS_VGUI7(".x%lx.c coords %lxSONOGRAM %d %d %d %d\n",
                  canvas, x,
                  x->x_xpos-1, x->x_ypos-1,
                  x->x_xpos+x->x_size*x->x_zoom+1,
                  x->x_ypos+x->x_blocksize/2*x->x_zoom+1);
        SYS_VGUI5(".x%lx.c coords ISONIMAGE%lx %d %d\n",
                  canvas, x,
                  x->x_xpos+((x->x_size*x->x_zoom)/2),
                  (x->x_ypos+((x->x_blocksize/2*x->x_zoom)/2)) );
        if ( x->x_phaso )
        {
            SYS_VGUI7(".x%lx.c coords %lxPHASOGRAM %d %d %d %d\n",
                      canvas, x,
                      x->x_xpos-1, x->x_ypos+(x->x_blocksize/2*x->x_zoom)+1,
                      x->x_xpos+x->x_size*x->x_zoom+1,
                      x->x_ypos+x->x_blocksize*x->x_zoom+3);
            SYS_VGUI5(".x%lx.c coords IFAZIMAGE%lx %d %d\n",
                      canvas, x,
                      x->x_xpos+((x->x_size*x->x_zoom)/2),
...
static void sonogram_draw_select(t_sonogram* x,t_glist* glist)
{
    ...
    if ( x->x_graphic && glist_isvisible( x->x_glist ) )
    {
        if(x->x_selected)
        {
            ...
            SYS_VGUI3(".x%lx.c itemconfigure %lxSONOGRAM -outline #0000FF\n", canvas, x);
            if ( x->x_phaso )
            {
                SYS_VGUI3(".x%lx.c itemconfigure %lxPHASOGRAM -outline #0000FF\n", canvas, x);
            }
        }
        else
        {
...
static void sonogram_motion(t_sonogram *x, t_floatarg dx, t_floatarg dy)
{
    ...
    else
    {
        ...
        SYS_VGUI3( ".x%lx.c delete %lxCAPTURE\n", canvas, x );
        SYS_VGUI7( ".x%lx.c create rectangle %d %d %d %d -outline #0000FF -tags %lxCAPTURE\n",
...
static void sonogram_readstart(t_sonogram *x, t_floatarg fstart)
{
    ...
    if ( x->x_graphic && glist_isvisible( x->x_glist ) )
    {
        SYS_VGUI3( ".x%lx.c delete line %lxREADSTART\n",
                   canvas, x);
        SYS_VGUI7( ".x%lx.c create line %d %d %d %d -fill #FF0000 -tags %lxREADSTART -width 3\n",
...
static void sonogram_modstart(t_sonogram *x, t_floatarg fstart)
{
    ...
    if ( x->x_graphic && glist_isvisible( x->x_glist ) )
    {
        SYS_VGUI3( ".x%lx.c delete line %lxMODSTART\n",
                   canvas, x);
        SYS_VGUI7( ".x%lx.c create line %d %d %d %d -fill #11E834 -tags %lxMODSTART -width 3\n",
...
static void sonogram_readend(t_sonogram *x, t_floatarg fend)
{
    ...
    if ( x->x_graphic && glist_isvisible( x->x_glist ) )
    {
        SYS_VGUI3( ".x%lx.c delete line %lxREADEND\n",
                   canvas, x);
        SYS_VGUI7( ".x%lx.c create line %d %d %d %d -fill #FF0000 -tags %lxREADEND -width 3\n",
...
static void sonogram_modend(t_sonogram *x, t_floatarg fend)
{
    ...
    if ( x->x_graphic && glist_isvisible( x->x_glist ) )
    {
        SYS_VGUI3( ".x%lx.c delete line %lxMODEND\n",
                   canvas, x);
        SYS_VGUI7( ".x%lx.c create line %d %d %d %d -fill #11E834 -tags %lxMODEND -width 3\n",
