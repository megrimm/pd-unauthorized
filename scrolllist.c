...
static t_scrolllist *scrolllist_new(t_symbol *s, int argc, t_atom *argv )
{
    ...
    if ( argc >= 1 )
    {
        ...
        {
            pd_error( "scrolllist : wrong argument (capacity : 1)", NULL );
            return NULL;
        }
    ...
    if ( argc >= 2 )
    {
        ...
        {
            pd_error( "scrolllist : wrong argument (width : 2)", NULL );
            return NULL;
        }
        if ( (int)argv[1].a_w.w_float <= 0 )
        {
            pd_error( "scrolllist : wrong width (%d)", (int)argv[1].a_w.w_float );
            pd_error( "scrolllist : usage : scrolllist <capacity> <width> <height>", NULL );
            return NULL;
        }
        x->x_width = (int)argv[1].a_w.w_float;
    }
    if ( argc >= 3 )
    {
        ...
        {
            pd_error( "scrolllist : wrong argument (height : 3)", NULL );
            return NULL;
        }
        if ( (int)argv[2].a_w.w_float <= 0 )
        {
            pd_error( "scrolllist : wrong height (%d)", (int)argv[2].a_w.w_float );
            pd_error( "scrolllist : usage : scrolllist <capacity> <width> <height>", NULL );
            return NULL;
        }
        x->x_height = (int)argv[2].a_w.w_float;
    ...
    if ( argc >= 6 )
    {
        ...
        {
            pd_error( "scrolllist : wrong arguments (font : 4,6)", NULL );
            pd_error( "argument types : %d %d", argv[3].a_type, argv[5].a_type );
            return NULL;
        }
        if ( argv[4].a_type != A_SYMBOL &&
                argv[4].a_type != A_FLOAT )
        {
            pd_error( "scrolllist : wrong arguments (font size : 5)", NULL );
            pd_error( "argument types : %d", argv[4].a_type );
            return NULL;
        }
        if ( argv[4].a_type == A_SYMBOL )
        {
    ...
    if ( argc >= 7-argoffset )
    {
        ...
        {
            pd_error( "scrolllist : wrong arguments (background color : %d)", (int)(7-argoffset) );
            return NULL;
        }
    ...
    if ( argc >= 8-argoffset )
    {
        ...
        {
            pd_error( "scrolllist : wrong arguments (foreground color : %d)", (int)(8-argoffset) );
            return NULL;
        }
    ...
    if ( argc >= 9-argoffset )
    {
        ...
        {
            pd_error( "scrolllist : wrong arguments (selection color : %d)", (int)(9-argoffset) );
            return NULL;
        }
